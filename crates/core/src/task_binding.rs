...
/// still stored — the only positional evidence a run carries. `None` skips the
/// positional signal entirely.
pub fn resolve(
    tasks: &[ParsedTask],
    stored_anchor: &str,
    ...
    let by_text = match_exact(tasks, stored_anchor)
        .map(|i| (i, MatchKind::Exact))
        .or_else(|| match_legacy_text(tasks, stored_anchor).map(|i| (i, MatchKind::LegacyText)));
