// The live run view's freshness stamp and agent status pill. Portals into
// the shell's toolbar (`.toolbar__filter`, see Toolbar) so the run view has
// exactly one control strip instead of the toolbar plus a redundant per-tab
// bar underneath it. Every slot is optional — a task tab has no event stream
// (no freshness) — so only the pieces the host supplies render. It is purely
// presentational; agent status and freshness source both live in the host.
//
